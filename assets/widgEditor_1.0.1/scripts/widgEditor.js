...
widgSelectBlockOptions.push("", "Ganti tipe blok");
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		...
		{
			case "bold":
				this.addButton(this.theList.id + "ButtonBold", "widgButtonBold fa fa-bold", "", "bold");
				
				break;
				
			case "italic":
				this.addButton(this.theList.id + "ButtonItalic", "widgButtonItalic fa fa-italic", "", "italic");
				
				break;

			case "underline":
				this.addButton(this.theList.id + "ButtonUnderline", "widgButtonUnderline fa fa-underline", "", "underline");
			...
			/*case "unorderedlist":
				this.addButton(this.theList.id + "ButtonUnordered", "widgButtonUnordered fa fa-list", "", "insertunorderedlist");
				
				break;
				
			case "orderedlist":
				this.addButton(this.theList.id + "ButtonOrdered", "widgButtonOrdered fa fa-list-ol", "", "insertorderedlist");
				
				break;*/
				
			case "image":
				this.addButton(this.theList.id + "ButtonImage", "widgButtonImage fa fa-image fa-lg", "", "image");
			...
			case "savedoc":
				this.addButton(this.theList.id + "ButtonSaveDoc", "widgButtonSaveDoc fa fa-save fa-lg", "", "savedoc");
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "image":
			var
			imgsrc = '';
			$(".container").append('<input type="file" class="img-upload">');
			$(".img-upload").css('display', 'none');
			$(".img-upload").trigger('click');
			$(".img-upload").change(function(){
				var
				valueArr = $(this).val().split('\\');
				imgsrc = valueArr[valueArr.length-1];
			});
			console.log($(".img-upload").val());
			$(".img-upload").change(function(){
				var theImage = baseUrlReplace+"/assets/uploads/"+imgsrc;
				var imgNameSplit = imgsrc.split('.');
				
				//next is create regex to find spesific text with first parent 'body'
				/*
					RegExp to find specific text with first parent 'body':
					 var str = "<body>outer<b>inner</b><b>inners</b>";
  					 var patt1 = /<(.?)>[a-zA-Z]+<\/(.?)>/g;
   					 var result = str.match(patt1);
   					 var strs = result[0];
   					 var results = strs.match(/<(.?)>/);
   					 var fresult;
   					 if (results[1] == 'b') {
    					fresult = strs.match(/<b>(.*)<\/b>/);
   					 } else {
    					fresult = 'no result';
   					 }
    					document.getElementById("demo").innerHTML = result;
    					console.log(fresult[1]);
				 */
				// don't forget to add show/hide function for recovery id
				// don't forget to remove login_status function
				// don't fotget to add multiple layout choice for pdf layout
			if (theImage != null && theImage != "")
			{
				var theAlt = imgNameSplit[0];
		...
		case 'savedoc':
			...
			availableEndTags = ['</p>', '</b>', '</em>', '</u>', '</img>', '</div>', '</ul>', '</ol>', '</span>', '</style>'],
			noTagText = '',
			matchingPattern = [],
			matchingObject = null,
			tags = [];
			...
			console.log('Join Data: ', pDFDocAD);
			console.log('Tags: ', pDFDocAD[0]);
			console.log('PlainText: ', pDFDocAD[1]);
			console.log('spaceChar: ', pDFDocAD[2]);
			console.log('Firts Parent: ', pDFDocAD[3]);
			console.log('Content Order List: ', pDFDocAD[4]);
			console.log('Data Result: ', pDFDocAD[5]);
			console.log('First Text: ', pDFDocAD[6]);

			var
			token = $(".sk-new-entry-form").data('token'),
			noSurat = $(".sk-new-entry-form input[name=no_surat]").val(),
			forValue = $(".sk-new-entry-form input[name=user]").val(),
			pDFDocAD = createPDFDocAD(matchingObject);
			pDFDocAD = pDFDocAD.join('|');
			$(".sk-new-entry-form input[name=editor_data]").val(pDFDocAD);
			$.ajax({
				url: baseURL() + '/save_editor_data',
				type: 'POST',
				data: {editor_data: iframeData},
			})
			.done(function() {
				console.log("success");
			})
			...
			/**
			 * special characher and sensitive word converter (check and convert), convert word or special character like
			 * span, style, coma(,) and another special character
			 * make the pdf data converter function understand a string data and fix a bug when function is executed and return the result
			 
			console.log(sCaSWConverter('you will be Style, loadout ,ads. Arabic'));
			function sCaSWConverter(str) {
				var
				chars = str.match(/([!"#$%&'()*+,./:;<=>?@[\]^`{|}~])/g),
				...
				for (; i < charsCount; i++) {
					switch(chars[i]) {
						case ',':
							str = str.replace(/([,])/g, 'esccoma');
							break;
						case '.':
							str = str.replace(/([.])/g, 'dcoma');
							break;
						default:
							break;
					}
			...
			var
			replstr = sCaSWConverter('you will be Style, loadout ,ads. Arabic');
			console.log();
			*/
